...
class LegacyMaskedLmLoss(FairseqCriterion):
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar('loss', agg_loss / sample_size / math.log(2) if sample_size > 0 else 0., round=3)
        metrics.log_derived('ppl', lambda meters: utils.get_perplexity(meters['loss'].avg))
        metrics.log_scalar('lm_loss', lm_loss_sum / ntokens / math.log(2) if ntokens > 0 else 0., round=3)
        metrics.log_scalar('sentence_loss', sentence_loss_sum / nsentences / math.log(2) if nsentences > 0 else 0., round=3)
        metrics.log_scalar('nll_loss', lm_loss_sum / ntokens / math.log(2) if ntokens > 0 else 0., round=3)
